...
# Load the data
Quantil_final_final <- read.csv(file = "/scicore/home/penny/masthi00/WF_spread/Visulaise_results/Paper/Data_factors_effect_CSA_all.csv", header  = TRUE)
...
PLOT <- ggplot(data = Quantil_final_4) +
  ...
  scale_color_manual(
    ...
    labels = c("Fitness cost", "Half-life drug A (days)", "Half-life drug B (days)", "Cmax/EC50 drug A", "Cmax/EC50 drug B")) +
...
ggsave("/scicore/home/penny/masthi00/WF_spread/Visulaise_results/Paper/Figure 3.pdf",
       plot = PLOT, width = 15, height = 18, device="pdf", units = "cm", dpi = 300)
