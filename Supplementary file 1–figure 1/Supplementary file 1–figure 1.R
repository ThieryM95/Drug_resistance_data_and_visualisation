...
# Load data
X <- read.csv(file = "/scicore/home/penny/masthi00/WF_spread/Visulaise_results/Paper/Data_prediction_T25_ACT.csv", header  = TRUE)
...
PA <- ggplot(data = X2, aes(
  ...
  xlab("") +
  theme_bw() +
  scale_y_continuous("T25 (years)", lim = c(0, 60), sec.axis = sec_axis(~ exp((-1.058766)+ . * (-0.2172797) + 0.002650265  * .), breaks = c(0.00001, 0.0001, 0.001, 0.01, 0.1), name = "Probability of establishment")) +
  theme(
    legend.text = element_text(size = 16/constant),
    legend.title = element_text(size = 16/constant, color = "black", face = "bold"),
    axis.text.x = element_text(size = 15/constant, color = "black"),
    axis.text.y = element_text(size = 15/constant, color = "black", margin = margin(t = 0, r = 0, b = 0, l = 4)),
    ...
    axis.text.y.right = element_text(size = 15/constant, color = "gray30", margin = margin(t = 0, r = 4, b = 0, l = 0))) +
...
# ---- Visualize for setting with a low EIR ----

# Select the data
X2 <- X[X$eir == 500, ]

# Define variable that need to be a factor
X2$Resistance_Level_short <- as.factor(X2$Resistance_Level_short)
X2$eir <- as.factor(X2$eir)
...
PB <- ggplot(data = X2, aes(
  ...
  ylab("") +
  theme_bw() +
  scale_y_continuous("T25 (years)", lim = c(0, 60), sec.axis = sec_axis(~ exp((-1.453703) + . * (-0.1571547) + 0.001756851 * .), breaks = c(0.00001, 0.0001, 0.001, 0.01, 0.1), name = "Probability of establishment")) +
  theme(
    legend.text = element_text(size = 16/constant),
    legend.title = element_text(size = 16/constant, color = "black", face = "bold"),
    axis.text.x = element_text(size = 15/constant, color = "black"),
    ...
    axis.text.y.right = element_text(size = 15/constant, color = "gray30", margin = margin(t = 0, r = 4, b = 0, l = 0))) +
  theme(strip.text.x = element_text(size = 16/constant, color = "black", face = "bold")) +
  labs(col = "Degree of resistance\n to drug A:") +
  theme(plot.margin = unit(c(0.05, 0.05, 0.05, 0.1), "cm")) +
...
# Merge the plot
PLOT <- plot_grid(PA, PB, ncol = 2, nrow = 1, rel_widths = c(1, 1.6), scale = 1)
PLOT<-ggdraw(add_sub(PLOT, "Degree of resistance to drug B", vpadding = grid::unit(0, "lines"), y = 6, x = 0.4, vjust = 4.5, size = 16/constant, fontface = "bold"))


ggsave("/scicore/home/penny/masthi00/WF_spread/Visulaise_results/Paper/Supplementary file 1–figure 1.pdf",
       plot = PLOT, width = 16, height = 6, device="pdf", units = "cm", dpi = 300)
